...
import "./index.less"
import { Tree, Input, Tag, message, Divider, Button } from 'antd';
...
class TreeTransfer extends Component {
	...
	onOkClick = () => {
		...
		const optionsNames = selectedKeys.map(one => {
				...
				<div className="tree-transfer-left">
					<div className="hot-city">
						{options.map((one, index) => {
							return <CheckableTag checked={checkedKeys.includes(one.key)} onChange={this.onHotTagClick(one.key)} key={index}>{one.title}</CheckableTag>
						})}
					</div>
					<Divider style={{ marginBottom: 0, marginTop: 16 }} />
					{/* <Search style={{ margin: "10px auto" }} placeholder="Search" onChange={this.onChangeSearch} /> */}
					...
					<div className="left-footer">
						<Button onClick={this.onClickCancel} style={{ marginRight: 10 }}>取消</Button>
						<Button type='primary' disabled={selectedKeys.length == 0} onClick={this.onOkClick}>确定</Button>
					</div>
				</div>
				<div className="tree-transfer-right">
					<div className="right-title">
						<span>已选结果</span>
						<a onClick={() => { this.setState({ selectedKeys: [], checkedKeys: [] }) }}>清空</a>
					</div>
